...
export default function CadastroProdutoScreen() {
  ...
  const validarDescricao = (val: string) => {
    setDescricao(val);
    setErroDescricao(val.trim() ? '' : 'Descrição é obrigatória');
  };

  const validarCategoria = (val: string) => {
    setCategoria(val);
  ...
  const validarPreco = (val: string) => {
    ...
    } else if (isNaN(paraNumero(val)) || paraNumero(val) <= 0) {
  ...
  const validarQuantidade = (val: string) => {
    setQuantidade(val);
    if (!val.trim()) {
      setErroQuantidade('Informe a quantidade do produto');
    } else if (!/^\d+$/.test(val.trim()) || parseInt(val, 10) <= 0) {
  ...
  const formValido =
    ...
    descricao.trim() !== '' &&
    categoria.trim() !== '' &&
    preco.trim() !== '' && !isNaN(paraNumero(preco)) && paraNumero(preco) > 0 &&
    /^\d+$/.test(quantidade.trim()) && parseInt(quantidade, 10) > 0 &&
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <InputField
        label="Categoria *"
        placeholder="Ex: Informática, Material Escolar, Eletrônicos"
        value={categoria}
