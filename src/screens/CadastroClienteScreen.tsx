...
export default function CadastroClienteScreen() {
  ...
  const validarNome = (val: string) => {
    setNome(val);
    setErroNome(val.trim() ? '' : 'Nome é obrigatório');
  };
  ...
  const validarEndereco = (val: string) => {
    setEndereco(val);
    setErroEndereco(val.trim() ? '' : 'Endereço é obrigatório');
  };

  const validarTelefone = (val: string) => {
    setTelefone(val);
  ...
  const validarCidade = (val: string) => {
    setCidade(val);
    setErroCidade(val.trim() ? '' : 'Cidade é obrigatória');
  };

  const validarBairro = (val: string) => {
    setBairro(val);
    setErroBairro(val.trim() ? '' : 'Bairro é obrigatório');
  };

  const validarEstado = (val: string) => {
    setEstado(val);
    setErroEstado(val.trim() ? '' : 'Estado é obrigatório');
  };
  ...
  const formValido =
    ...
    bairro.trim() !== '' &&
    estado.trim() !== '' &&
    !erroNome && !erroEmail && !erroEndereco && !erroTelefone &&
  ...
  return (
    ...
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <InputField
        label="Endereço *"
        placeholder="Rua das Flores, 120, Pernambuco"
        value={endereco}
        onChangeText={validarEndereco}
        onBlur={() => validarEndereco(endereco)}
        error={erroEndereco}
      />
      <InputField
        label="Telefone *"
